...
// --- Record ---
struct EcRecord {
    std::vector<uint8_t> canonical_public_key;   // P-256 uncompressed 65B
...
struct EcRequestDescription {
    ...
    std::vector<uint8_t> verification_key;        // 65B uncompressed
...
EcRequestDescription assemble_ec_request(
    const EcRecord& rec,
...
Result<EcRequestResult> encode_ec_request(
    const EcRequestDescription& desc,
...
Result<EcResponse> decode_ec_response(
