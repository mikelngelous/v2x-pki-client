...
protected:
    ...
    static void SetUpTestSuite() {
        ...
        auto aa_cert = cert_utils::build_root_cert("aa_test", aa->public_key, aa->private_key);
        if (!aa_cert) return;
        aa_cert_ = *aa_cert;

        auto ec_cert = cert_utils::build_signed_cert("ec_test", ec->public_key, ea->private_key,
...
TEST_F(AtRequestTest, EncodeProducesBytes) {
    ...
    auto desc = assemble_at_request(rec, std::chrono::system_clock::now());
    auto encoded = encode_at_request(desc, aa_cert_, ea_cert_, ec_cert_, ec_keys_.private_key,
                                     at_keys_.private_key);
...
TEST_F(AtRequestTest, EncodeBadAa) {
    ...
    auto encoded = encode_at_request(desc, bad_aa, ea_cert_, ec_cert_, ec_keys_.private_key,
                                     at_keys_.private_key);
